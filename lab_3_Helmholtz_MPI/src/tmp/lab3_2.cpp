							   ...
							   // - MPI_Send_init, MPI_Recv_init
{
	...
	if (rank == 0) {
		...
		/*	for (size_t i = 0; i < m; i++) //собираем правую часть и точное
		   решение points[i] = i * h; for (size_t i = 0; i < m; i++) for (size_t
		   j = 0; j < m; j++)
				{
	...
	MPI_Status st;
	double denominator = 1 / (4 + h * h * k * k);
	double N = m * m;
							   ...
							   // - MPI_Send_init, MPI_Recv_init
{
	...
	if (rank == 0) {
		...
		/*for (size_t i = 0; i < m; i++)                             //собираем
		правую часть и точное решение points[i] = i * h; for (size_t i = 0; i <
		m; i++) for (size_t j = 0; j < m; j++)
			{
				rhsGlob[i * m + j] = h * h * f(points[i], points[j]);
				exactsol[i * m + j] = u(points[i], points[j]);
			}*/
	...
	do {
		...
		for (size_t i = 0; i < m; i++)
			for (size_t j = 0; j < m; j++) {
				rhs[i * m + j] = h * h * f(points[i], points[j]);
				exactsol[i * m + j] = u(points[i], points[j]);
			}
