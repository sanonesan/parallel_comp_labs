...
double DifNorm(const vector<double>& A,const vector<double>& B)
{
    ...
    double dd = 0.0;
    int size = A.size();
#pragma omp parallel for default(none) shared(A,B,size) private(dd)  reduction(+:norm)
        ...
        for (int j = 1; j < n - 1; ++j)
            for (int i = 1; i < n - 1; ++i)
                u[j * n + i] = (h2 * f(h * i, h * j) + u0[j * n + i - 1] \
                    + u0[j * n + i + 1] + u0[(j - 1) * n + i] + u0[(j + 1) * n + i]) / koef;
        ...
        for (int j = 1; j < n - 1; ++j)
            for (int i = 1; i < n - 1; ++i)
                u[j * n + i] = (h2 * f(h * i, h * j) + u[j * n + i - 1] \
                    + u0[j * n + i + 1] + u[(j-1) * n + i] + u0[(j+1) * n  + i]) / koef;
...
#pragma omp parallel for default(none) shared(u,h,h2,n,koef)
        for (int j = 1; j < n - 1; ++j)
            for (int i = 1 + j % 2; i < n - 1; i += 2)
                u[j * n + i] = (h2 * f(h * i, h * j) + u[j * n + i - 1] \
                    + u[j * n + i + 1] + u[(j - 1) * n + i] + u[(j + 1) * n + i]) / koef;
